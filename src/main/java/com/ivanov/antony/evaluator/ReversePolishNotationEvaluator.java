...
public class ReversePolishNotationEvaluator implements Evaluator {
    ...
    public double evaluate(List<Node> polishNotation) {
        Deque<Double> loader = new LinkedList<>(); // stack to hold numbers and results
        for (Node notationNode : polishNotation) { // cycle through outputList
        ...
        return loader.removeFirst();// return the last item on the stack = result
    }
}
